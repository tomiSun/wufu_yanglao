...
import { sendSmsByWeiMai } from '@/services/User/login';
...
const LoginItem = (props) => {
  ...
  const onGetCaptcha = useCallback(async (mobile) => {
    const result = await sendSmsByWeiMai(mobile);
    ...
    return (
      <FormItem shouldUpdate noStyle>
        {({ getFieldValue }) => (
          <Row gutter={8}>
            <Col span={16}>
              <FormItem label={props?.label} name={name} {...options}>
